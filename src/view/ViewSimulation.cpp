...
namespace view
{
    ...
    void view::ViewSimulation::dessinerAeroportVueArriere(sf::RenderWindow &window)
    {
        ...
        // gerer la taille de l aeroport (pour la distance)
        ...
        float positionYparRaportAvion = 0;

        if (jeu.getAltitudeAvionAeroport() >= HAUTEUR_MIN_VISION)
        {
            positionYparRaportAvion = HAUTEUR_MIN_VISION;
        }

        if (jeu.getAltitudeAvionAeroport() < HAUTEUR_MIN_VISION && jeu.getAltitudeAvionAeroport() >= 0)
        {
            positionYparRaportAvion = jeu.getAltitudeAvionAeroport() * HAUTEUR_VISION_PIXIEL / HAUTEUR_MIN_VISION;
        }

        float positionYaeroport = this->avionSprite.getPosition().y + positionYparRaportAvion;
        this->aeroportRect.setSize(sf::Vector2f(tailleAeroport, 100));
        this->aeroportRect.setPosition(positionCenterX(window, tailleAeroport), positionYaeroport);
        std::cout << "position aeroport par raport a av   : " << positionYparRaportAvion << std::endl;
